...
pub mod accounts {
	...
	pub fn init_balances() -> Vec<AccountId> {
		vec![
			...
			tangle_kusama_runtime::TreasuryPalletId::get().into_account_truncating(),
		]
	}
}
		...
		let genesis_config = polkadot_runtime::RuntimeGenesisConfig {
			...
			paras: polkadot_runtime::ParasConfig {
				paras: vec![
					...
					// (
					// 	tangle_polkadot::PARA_ID.into(),
					// 	ParaGenesisArgs {
					// 		genesis_head: HeadData::default(),
					// 		validation_code: ValidationCode(
					// 			tangle_polkadot_runtime::WASM_BINARY.unwrap().to_vec(),
					// 		),
					// 		para_kind: ParaKind::Parachain,
					// 	},
					// ),
		...
		let genesis_config = kusama_runtime::RuntimeGenesisConfig {
			...
			paras: kusama_runtime::ParasConfig {
				paras: vec![
					...
					(
						tangle_kusama::PARA_ID.into(),
						ParaGenesisArgs {
							genesis_head: HeadData::default(),
							validation_code: ValidationCode(
								tangle_kusama_runtime::WASM_BINARY.unwrap().to_vec(),
							),
							para_kind: ParaKind::Parachain,
						},
					),
...
pub mod tangle_kusama {
	use super::*;
	use crate::ALICE;
	use tangle_primitives::KSM;

	pub const PARA_ID: u32 = 2001;
	pub const ED: Balance = 1_000_000_000_000;

	pub fn genesis() -> Storage {
		let genesis_config = tangle_kusama_runtime::RuntimeGenesisConfig {
			system: tangle_kusama_runtime::SystemConfig {
				code: tangle_kusama_runtime::WASM_BINARY
					.expect("WASM binary was not build, please build it!")
					.to_vec(),
				..Default::default()
			},
			balances: tangle_kusama_runtime::BalancesConfig {
				balances: accounts::init_balances()
					.iter()
					.cloned()
			...
			parachain_info: tangle_kusama_runtime::ParachainInfoConfig {
				parachain_id: PARA_ID.into(),
				..Default::default()
			},
			session: tangle_kusama_runtime::SessionConfig {
				keys: collators::invulnerables()
					.into_iter()
					.map(|(acc, aura)| {
						(
							acc.clone(),                                 // account id
							acc,                                         // validator id
							tangle_kusama_runtime::SessionKeys { aura }, // session keys
						)
					})
					.collect(),
			},
			// tangle_parachain_staking: tangle_kusama_runtime::ParachainStakingConfig {
			// 	candidates: collators::candidates(),
			// 	delegations: collators::delegations(),
			// 	inflation_config: collators::inflation_config(),
			// },
			polkadot_xcm: tangle_kusama_runtime::PolkadotXcmConfig {
				safe_xcm_version: Some(SAFE_XCM_VERSION),
				..Default::default()
			},
			tokens: tangle_kusama_runtime::TokensConfig {
				balances: vec![(
					get_account_id_from_seed::<sr25519::Public>(ALICE),
					KSM,
			...
			asset_registry: tangle_kusama_runtime::AssetRegistryConfig {
