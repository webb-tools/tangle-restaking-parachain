...
use tangle_slp::{primitives::UnlockChunk, Delays, Ledger, MinimumsMaximums, SubstrateLedger};
...
const tangle_TREASURY_MULTILOCATION: MultiLocation = MultiLocation {
	parents: 0,
	interior: X1(AccountId32 { network: None, id: tangle_TREASURY_ACCOUNT }),
};
...
fn slp_setup() {
	cross_ksm_to_tangle(tangle_TREASURY_ACCOUNT, 10000 * KSM_DECIMALS);
	// cross_ksm_to_tangle(ENTRANCE_ACCOUNT, 10000 * KSM_DECIMALS);
	cross_ksm_to_tangle(ALICE, 10000 * KSM_DECIMALS);
	cross_ksm_to_tangle(BOB, 10000 * KSM_DECIMALS);
	...
	TangleKusama::execute_with(|| {
		...
		assert_ok!(Slp::set_fee_source(
			RuntimeOrigin::root(),
			RelayCurrencyId::get(),
			Some((tangle_TREASURY_MULTILOCATION, 1 * KSM_DECIMALS))
		));
		// set fee_source for ksm to be treasury
		assert_ok!(Slp::set_fee_source(
			RuntimeOrigin::root(),
			NativeCurrencyId::get(),
			Some((tangle_TREASURY_MULTILOCATION, 1 * BNC_DECIMALS))
		));
		...
		assert_ok!(Slp::set_hosting_fees(
			RuntimeOrigin::root(),
			RelayCurrencyId::get(),
			Some((Permill::from_parts(1000), tangle_TREASURY_MULTILOCATION))
		));
		...
		assert_ok!(
			<Runtime as tangle_slp::Config>::XcmWeightAndFeeHandler::set_xcm_dest_weight_and_fee(
				RelayCurrencyId::get(),
		...
		tangle_kusama_runtime::System::reset_events();
	});
}

fn cross_ksm_to_tangle(to: [u8; 32], amount: u128) {
	Kusama::execute_with(|| {
		assert_ok!(kusama_runtime::Balances::force_set_balance(
			kusama_runtime::RuntimeOrigin::root(),
