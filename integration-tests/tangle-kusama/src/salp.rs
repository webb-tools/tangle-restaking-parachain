...
use tangle_kusama_runtime::{LeasePeriod, MinContribution, Runtime};
use tangle_salp::{Error, FundInfo, FundStatus};
...
fn redeem_with_speical_vsbond_should_work() {
	sp_io::TestExternalities::default().execute_with(|| {
		tangle::execute_with(|| {
			...
			let vs_token =
				<Runtime as tangle_salp::Config>::CurrencyIdConversion::convert_to_vstoken(
					RelayCurrencyId::get(),
				)
				.unwrap();
			let vs_bond =
				<Runtime as tangle_salp::Config>::CurrencyIdConversion::convert_to_vsbond(
					RelayCurrencyId::get(),
					2001,
					13,
					20,
