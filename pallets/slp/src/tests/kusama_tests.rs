...
fn refund_currency_due_unbond_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		tangle_lst_minting::OngoingTimeUnit::<Runtime>::insert(KSM, TimeUnit::Era(100));

		// Set TokenUnlockLedger records.
		let record_bob = (BOB, 10, TimeUnit::Era(90), RedeemType::Native);
		tangle_lst_minting::TokenUnlockLedger::<Runtime>::insert(KSM, 0, record_bob);

		let record_charlie = (CHARLIE, 28, TimeUnit::Era(100), RedeemType::Native);
		tangle_lst_minting::TokenUnlockLedger::<Runtime>::insert(KSM, 1, record_charlie);

		let record_dave = (DAVE, 30, TimeUnit::Era(100), RedeemType::Native);
		tangle_lst_minting::TokenUnlockLedger::<Runtime>::insert(KSM, 2, record_dave);

		let record_eddie_1 = (EDDIE, 7, TimeUnit::Era(110), RedeemType::Native);
		tangle_lst_minting::TokenUnlockLedger::<Runtime>::insert(KSM, 3, record_eddie_1);

		let record_eddie_2 = (EDDIE, 6, TimeUnit::Era(110), RedeemType::Native);
		tangle_lst_minting::TokenUnlockLedger::<Runtime>::insert(KSM, 4, record_eddie_2);
		...
		tangle_lst_minting::TimeUnitUnlockLedger::<Runtime>::insert(
			TimeUnit::Era(90),
			KSM,
		...
		tangle_lst_minting::UserUnlockLedger::<Runtime>::insert(
			BOB,
			KSM,
		...
		tangle_lst_minting::UnlockingTotal::<Runtime>::insert(KSM, 1000);
		...
		assert_eq!(tangle_lst_minting::TokenPool::<Runtime>::get(KSM), 0);
		...
		assert_eq!(
			tangle_lst_minting::TimeUnitUnlockLedger::<Runtime>::get(TimeUnit::Era(90), KSM,),
			Some(time_record_90)
		);
		assert_eq!(
			tangle_lst_minting::UserUnlockLedger::<Runtime>::get(BOB, KSM,),
		...
		assert_eq!(
			tangle_lst_minting::TokenUnlockLedger::<Runtime>::get(KSM, 2),
		...
		assert_eq!(
			tangle_lst_minting::TimeUnitUnlockLedger::<Runtime>::get(TimeUnit::Era(100), KSM,),
			Some(time_record_100_new)
		);

		assert_eq!(tangle_lst_minting::UserUnlockLedger::<Runtime>::get(CHARLIE, KSM,), None);
		assert_eq!(
			tangle_lst_minting::UserUnlockLedger::<Runtime>::get(DAVE, KSM,),
			Some((8, bounded_vec_dave))
		);

		// Unlocking records for era 110
		assert_eq!(
			tangle_lst_minting::TimeUnitUnlockLedger::<Runtime>::get(TimeUnit::Era(110), KSM,),
			Some(time_record_110)
		);

		assert_eq!(
			tangle_lst_minting::UserUnlockLedger::<Runtime>::get(EDDIE, KSM,),
		...
		// set era to 110
		tangle_lst_minting::OngoingTimeUnit::<Runtime>::insert(KSM, TimeUnit::Era(110));
		...
		assert_eq!(tangle_lst_minting::TokenUnlockLedger::<Runtime>::get(KSM, 3), None);
		assert_eq!(tangle_lst_minting::TokenUnlockLedger::<Runtime>::get(KSM, 4), None);

		assert_eq!(tangle_lst_minting::UserUnlockLedger::<Runtime>::get(EDDIE, KSM,), None);
