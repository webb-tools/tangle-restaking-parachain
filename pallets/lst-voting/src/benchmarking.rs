...
use tangle_primitives::{currency::VKSM, XcmOperationType as XcmOperation};
...
fn init_vote<T: Config>(lst: CurrencyIdOf<T>) -> Result<(), BenchmarkError> {
	...
	Pallet::<T>::set_undeciding_timeout(RawOrigin::Root.into(), lst, Zero::zero())?;
	Pallet::<T>::add_delegator(RawOrigin::Root.into(), lst, derivative_index)?;
	Pallet::<T>::set_vote_cap_ratio(RawOrigin::Root.into(), lst, Perbill::from_percent(10))?;
...
mod benchmarks {
	...
	fn vote_new() -> Result<(), BenchmarkError> {
		...
		for (i, index) in (0..T::MaxVotes::get()).collect::<Vec<_>>().iter().skip(1).enumerate() {
			...
			Pallet::<T>::vote(RawOrigin::Signed(caller.clone()).into(), lst, *index, vote)?;
		...
		#[extrinsic_call]
		Pallet::<T>::vote(RawOrigin::Signed(caller.clone()), lst, poll_index, vote);
	...
	fn vote_existing() -> Result<(), BenchmarkError> {
		...
		for index in (0..r).collect::<Vec<_>>().iter() {
			Pallet::<T>::vote(RawOrigin::Signed(caller.clone()).into(), lst, *index, old_vote)?;
			Pallet::<T>::notify_vote(
				control_origin.clone() as <T as frame_system::Config>::RuntimeOrigin,
				*index as QueryId,
				response.clone(),
		...
		#[extrinsic_call]
		Pallet::<T>::vote(RawOrigin::Signed(caller.clone()), lst, poll_index, new_vote);
	...
	pub fn unlock() -> Result<(), BenchmarkError> {
		...
		init_vote::<T>(lst)?;
		Pallet::<T>::vote(origin.clone().into(), lst, poll_index, vote)?;
		...
		Pallet::<T>::set_vote_locking_period(RawOrigin::Root.into(), lst, 0u32.into())?;
	...
	pub fn remove_delegator_vote() -> Result<(), BenchmarkError> {
		...
		#[extrinsic_call]
		_(origin, lst, poll_index, derivative_index);
	...
	pub fn kill_referendum() -> Result<(), BenchmarkError> {
		...
		Pallet::<T>::vote(origin_caller.into(), lst, poll_index, vote)?;
		Pallet::<T>::set_referendum_status(
			RawOrigin::Root.into(),
			lst,
			poll_index,
		...
		#[extrinsic_call]
		_(origin as <T as frame_system::Config>::RuntimeOrigin, lst, poll_index);
	...
	pub fn add_delegator() -> Result<(), BenchmarkError> {
		...
		T::DerivativeAccount::add_delegator(
			CurrencyId::to_token(&lst).unwrap(),
			derivative_index,
			Parent.into(),
		);

		#[extrinsic_call]
		_(origin as <T as frame_system::Config>::RuntimeOrigin, lst, derivative_index);
	...
	pub fn set_referendum_status() -> Result<(), BenchmarkError> {
		...
		#[extrinsic_call]
		_(origin as <T as frame_system::Config>::RuntimeOrigin, lst, poll_index, info);
	...
	pub fn set_undeciding_timeout() -> Result<(), BenchmarkError> {
		...
		#[extrinsic_call]
		_(origin as <T as frame_system::Config>::RuntimeOrigin, lst, vote_locking_period);
	...
	pub fn set_vote_locking_period() -> Result<(), BenchmarkError> {
		...
		#[extrinsic_call]
		_(origin as <T as frame_system::Config>::RuntimeOrigin, lst, undeciding_timeout);
	...
	pub fn set_vote_cap_ratio() -> Result<(), BenchmarkError> {
		...
		#[extrinsic_call]
		_(origin as <T as frame_system::Config>::RuntimeOrigin, lst, vote_cap_ratio);
