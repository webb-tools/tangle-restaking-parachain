	...
	impl<T: Config, C: Get<CurrencyIdOf<T>>> OnRuntimeUpgrade for MigrateToV2<T, C> {
		fn on_runtime_upgrade() -> Weight {
			if StorageVersion::get::<Pallet<T>>() < 2 {
				...
				log::info!("Migrating lst-voting storage to v2");
				StorageVersion::new(2).put::<Pallet<T>>();
				weight_consumed
			} else {
				log::warn!("lst-voting migration should be removed.");
		...
		fn pre_upgrade() -> Result<Vec<u8>, sp_runtime::DispatchError> {
			log::info!(
				"lst-voting before migration: version: {:?}",
				StorageVersion::get::<Pallet<T>>(),
			);
			log::info!(
				"lst-voting before migration: count: {}",
		...
		fn post_upgrade(_: Vec<u8>) -> Result<(), sp_runtime::DispatchError> {
			log::info!(
				"lst-voting after migration: version: {:?}",
				StorageVersion::get::<Pallet<T>>(),
			);
			log::info!(
				"lst-voting after migration: count: {}",
				v1::DelegatorVote::<T>::iter().count()
			);
			ensure!(
				v1::DelegatorVote::<T>::iter().count() == 0,
...
pub fn migrate_to_v2<T: Config, C: Get<CurrencyIdOf<T>>>() -> Weight {
	...
	let token = C::get();
	let lst = token.to_lst().unwrap();

	for who in ClassLocksFor::<T>::iter_keys() {
		let _ = T::MultiCurrency::remove_lock(CONVICTION_VOTING_ID, lst, &who);
	...
	impl<T: Config, C: Get<CurrencyIdOf<T>>> OnRuntimeUpgrade for MigrateToV3<T, C> {
		fn on_runtime_upgrade() -> Weight {
			if StorageVersion::get::<Pallet<T>>() == 2 {
				...
				log::info!("Migrating lst-voting storage to v3");
		...
		fn pre_upgrade() -> Result<Vec<u8>, sp_runtime::DispatchError> {
			...
			log::info!(
				"lst-voting before migration: ClassLocksFor v2 count: {}",
		...
		fn post_upgrade(_: Vec<u8>) -> Result<(), sp_runtime::DispatchError> {
			...
			log::info!(
				"lst-voting after migration: ClassLocksFor v3 count: {}",
...
pub fn migrate_to_v3<T: Config, C: Get<CurrencyIdOf<T>>>() -> Weight {
	...
	ClassLocksFor::<T>::translate::<Vec<(PollIndex, BalanceOf<T>)>, _>(
		|_: T::AccountId, locks: Vec<(PollIndex, BalanceOf<T>)>| {
			...
			weight += T::DbWeight::get().writes(1);
			Some(BoundedVec::try_from(vec![(lst, max_locked_balance)]).unwrap())
		},
	);
