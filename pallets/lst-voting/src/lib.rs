...
pub mod pallet {
	...
	impl<T: Config> BuildGenesisConfig for GenesisConfig<T> {
		fn build(&self) {
			let (lst, delegators) = &self.delegators;
			Delegators::<T>::insert(lst, BoundedVec::truncate_from(delegators.clone()));
			self.undeciding_timeouts.iter().for_each(|(lst, undeciding_timeout)| {
				UndecidingTimeout::<T>::insert(lst, undeciding_timeout);
			});
		...
		fn on_idle(n: BlockNumberFor<T>, remaining_weight: Weight) -> Weight {
			...
			for relay_block_number in ReferendumTimeout::<T>::iter_keys() {
				if relay_current_block_number >= relay_block_number {
					...
					for (lst, poll_index) in info_list.iter() {
						ReferendumInfoFor::<T>::mutate(lst, poll_index, |maybe_info| {
							match maybe_info {
								Some(info) => {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn vote(
			...
			lst_vote: AccountVote<BalanceOf<T>>,
		) -> DispatchResult {
			let who = ensure_signed(origin)?;
			Self::ensure_lst(&lst)?;
			ensure!(UndecidingTimeout::<T>::contains_key(lst), Error::<T>::NoData);
			Self::ensure_no_pending_vote(lst, poll_index)?;

			let token_vote = Self::compute_token_vote(lst, lst_vote)?;

			// create referendum if not exist
			let mut submitted = false;
			if !ReferendumInfoFor::<T>::contains_key(lst, poll_index) {
				...
				Self::ensure_referendum_ongoing(lst, poll_index)?;
				submitted = true;
			}

			// record vote info
			let (maybe_old_vote, maybe_total_vote) =
				Self::try_vote(&who, lst, poll_index, token_vote, lst_vote.balance())?;
			...
			let new_delegator_votes =
				Self::allocate_delegator_votes(lst, poll_index, delegator_total_vote)?;

			PendingDelegatorVotes::<T>::try_mutate(lst, poll_index, |item| -> DispatchResult {
			...
			let (weight, extra_fee) = T::XcmDestWeightAndFee::get_operation_weight_and_fee(
				CurrencyId::to_token(&lst).map_err(|_| Error::<T>::NoData)?,
			...
			Self::send_xcm_with_notify(
				...
				|query_id| {
					if !submitted {
						PendingReferendumInfo::<T>::insert(query_id, (lst, poll_index));
					}
					PendingVotingInfo::<T>::insert(
						query_id,
						(lst, poll_index, derivative_index, who.clone(), maybe_old_vote),
					)
				},
			)?;
		...
		pub fn unlock(
			...
			let who = ensure_signed(origin)?;
			Self::ensure_lst(&lst)?;
			Self::ensure_referendum_completed(lst, poll_index)
				.or(Self::ensure_referendum_killed(lst, poll_index))
				.map_err(|_| Error::<T>::NoPermissionYet)?;
			Self::ensure_no_pending_vote(lst, poll_index)?;

			Self::try_remove_vote(&who, lst, poll_index, UnvoteScope::OnlyExpired)?;
			Self::update_lock(&who, lst)?;

			Self::deposit_event(Event::<T>::Unlocked { who, lst, poll_index });
		...
		pub fn remove_delegator_vote(
			...
			let who = ensure_signed(origin)?;
			Self::ensure_lst(&lst)?;
			ensure!(DelegatorVotes::<T>::get(lst, poll_index).len() > 0, Error::<T>::NoData);
			Self::ensure_referendum_expired(lst, poll_index)?;
			...
			Self::send_xcm_with_notify(
				...
				|query_id| {
					PendingRemoveDelegatorVote::<T>::insert(
						query_id,
						(lst, poll_index, derivative_index),
					);
				},
			)?;

			Self::deposit_event(Event::<T>::DelegatorVoteRemoved { who, lst, derivative_index });
		...
		pub fn kill_referendum(
			...
			Self::deposit_event(Event::<T>::ReferendumKilled { lst, poll_index });
		...
		pub fn add_delegator(
			...
			let token = CurrencyId::to_token(&lst).map_err(|_| Error::<T>::NoData)?;
				...
				!Delegators::<T>::get(lst).contains(&derivative_index),
				Error::<T>::DerivativeIndexOccupied
			);

			Delegators::<T>::try_mutate(lst, |vec| -> DispatchResult {
				vec.try_push(derivative_index).map_err(|_| Error::<T>::TooMany)?;
				Ok(())
			})?;

			Self::deposit_event(Event::<T>::DelegatorAdded { lst, derivative_index });
		...
		pub fn set_referendum_status(
			...
			ensure!(ReferendumInfoFor::<T>::contains_key(lst, poll_index), Error::<T>::NoData);
			ReferendumInfoFor::<T>::insert(lst, poll_index, info.clone());

			Self::deposit_event(Event::<T>::ReferendumInfoSet { lst, poll_index, info });
		...
		pub fn set_vote_locking_period(
			...
			T::ControlOrigin::ensure_origin(origin)?;
			Self::ensure_lst(&lst)?;
			VoteLockingPeriod::<T>::insert(lst, locking_period);
			Self::deposit_event(Event::<T>::VoteLockingPeriodSet { lst, locking_period });
		...
		pub fn set_undeciding_timeout(
			...
			Self::ensure_lst(&lst)?;
			UndecidingTimeout::<T>::insert(lst, undeciding_timeout);
			Self::deposit_event(Event::<T>::UndecidingTimeoutSet { lst, undeciding_timeout });
		...
		pub fn notify_vote(
			...
			if let Some((lst, poll_index, derivative_index, who, maybe_old_vote)) =
				PendingVotingInfo::<T>::get(query_id)
			{
				if !success {
					// rollback vote
					let _ = PendingDelegatorVotes::<T>::clear(u32::MAX, None);
					Self::try_remove_vote(&who, lst, poll_index, UnvoteScope::Any)?;
					Self::update_lock(&who, lst)?;
					if let Some((old_vote, lst_balance)) = maybe_old_vote {
						Self::try_vote(&who, lst, poll_index, old_vote, lst_balance)?;
					}
				} else {
					if !VoteDelegatorFor::<T>::contains_key((&who, lst, poll_index)) {
						VoteDelegatorFor::<T>::insert((&who, lst, poll_index), derivative_index);
					}
					DelegatorVotes::<T>::remove(lst, poll_index);
					DelegatorVotes::<T>::try_mutate(lst, poll_index, |item| -> DispatchResult {
						for (derivative_index, vote) in
							PendingDelegatorVotes::<T>::take(lst, poll_index).iter()
						{
				...
				Self::deposit_event(Event::<T>::VoteNotified { lst, poll_index, success });
			}

			if let Some((lst, poll_index)) = PendingReferendumInfo::<T>::get(query_id) {
				if success {
								...
								if let ReferendumInfo::Ongoing(status) = info {
									...
									ReferendumTimeout::<T>::mutate(
										relay_current_block_number.saturating_add(
											UndecidingTimeout::<T>::get(lst)
												.ok_or(Error::<T>::NoData)?,
										),
										|ref_vec| {
											ref_vec
												.try_push((lst, poll_index))
												.map_err(|_| Error::<T>::TooMany)
										},
									)?;
					...
					ReferendumInfoFor::<T>::remove(lst, poll_index);
				}
		...
		pub fn notify_remove_delegator_vote(
			...
			let responder = Self::ensure_xcm_response_or_governance(origin)?;
			if let Some((lst, poll_index, _derivative_index)) =
		...
		pub fn set_vote_cap_ratio(
			...
			T::ControlOrigin::ensure_origin(origin)?;
			Self::ensure_lst(&lst)?;
			VoteCapRatio::<T>::insert(lst, vote_cap_ratio);
			Self::deposit_event(Event::<T>::VoteCapRatioSet { lst, vote_cap_ratio });
	...
	impl<T: Config> Pallet<T> {
		...
		> {
			...
			Self::try_access_poll(lst, poll_index, |poll_status| {
				...
				VotingFor::<T>::try_mutate(who, |voting| {
					...
					Self::set_lock(&who, lst, voting.locked_lst_balance())?;
		...
		pub(crate) fn try_remove_vote(
			...
			VotingFor::<T>::try_mutate(who, |voting| {
					...
					voting
				{
					...
					Self::try_access_poll(lst, poll_index, |poll_status| match poll_status {
						...
						PollStatus::Completed(end, approved) => {
							if let Some((lock_periods, _)) = v.1.locked_if(approved) {
								...
								if now < unlock_at {
									...
									// v.3 is the actual locked lst balance
		...
		pub(crate) fn update_lock(who: &AccountIdOf<T>, lst: CurrencyIdOf<T>) -> DispatchResult {
			...
			if lock_needed.is_zero() {
				ClassLocksFor::<T>::mutate(who, |locks| {
					locks.retain(|x| x.0 != lst);
				});
				T::MultiCurrency::remove_lock(CONVICTION_VOTING_ID, lst, who)
			} else {
				ClassLocksFor::<T>::mutate(who, |locks| {
					match locks.iter().position(|x| x.0 == lst) {
						Some(i) => locks[i].1 = lock_needed,
						None => {
							let ok = locks.try_push((lst, lock_needed)).is_ok();
							debug_assert!(
								ok,
				...
				T::MultiCurrency::set_lock(CONVICTION_VOTING_ID, lst, who, lock_needed)
			}
		}
		...
		pub(crate) fn set_lock(
			...
			ClassLocksFor::<T>::mutate(who, |locks| match locks.iter().position(|x| x.0 == lst) {
				Some(i) => locks[i].1 = amount,
				None => {
					let ok = locks.try_push((lst, amount)).is_ok();
					debug_assert!(
						ok,
						"Vec bounded by number of classes; \
						all items in Vec associated with a unique class; \
			...
			if amount.is_zero() {
				...
				T::MultiCurrency::set_lock(CONVICTION_VOTING_ID, lst, who, amount)
			}
		}
		...
		fn ensure_no_pending_vote(lst: CurrencyIdOf<T>, poll_index: PollIndex) -> DispatchResult {
			ensure!(
				!PendingVotingInfo::<T>::iter()
					.any(|(_, (v, p, _, _, _))| v == lst && p == poll_index),
				Error::<T>::PendingVote
			);
			Ok(())
		}
		...
		pub fn ensure_referendum_ongoing(
			...
			match ReferendumInfoFor::<T>::get(lst, poll_index) {
		...
		fn ensure_referendum_expired(
			...
			let delegator_votes = DelegatorVotes::<T>::get(lst, poll_index).into_inner();
			let (_derivative_index, delegator_vote) =
				delegator_votes.first().ok_or(Error::<T>::NoData)?;
			match (ReferendumInfoFor::<T>::get(lst, poll_index), delegator_vote.locked_if(true)) {
				(Some(ReferendumInfo::Completed(moment)), Some((lock_periods, _balance))) => {
					let locking_period =
						VoteLockingPeriod::<T>::get(lst).ok_or(Error::<T>::NoData)?;
					ensure!(
						T::RelaychainBlockNumberProvider::current_block_number()
							>= moment
		...
		fn ensure_referendum_killed(lst: CurrencyIdOf<T>, poll_index: PollIndex) -> DispatchResult {
			match ReferendumInfoFor::<T>::get(lst, poll_index) {
		...
		fn compute_token_vote(
			...
			new_vote
				...
				.and_then(|_| new_vote.checked_div(lst_supply))?;
		...
		pub(crate) fn compute_delegator_total_vote(
			...
			let conviction_votes = vote
				...
				.votes(vote.balance())
				.votes;
			let vote_cap = Self::vote_cap(lst)?;
			for i in 0..=6 {
				let conviction =
		...
		pub(crate) fn allocate_delegator_votes(
			...
			let token = CurrencyId::to_token(&lst).map_err(|_| Error::<T>::NoData)?;
			let mut delegator_votes = DelegatorVotes::<T>::get(lst, poll_index).into_inner();
			let delegator_vote_keys =
				delegator_votes.iter().map(|(index, _)| *index).collect::<Vec<_>>();
			for derivative_index in Delegators::<T>::get(lst) {
