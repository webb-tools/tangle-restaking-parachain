...
parameter_types! {
	pub static lstSupply: Balance = u64::MAX.into();
	pub static TokenSupply: Balance = u64::MAX.into();
}

pub struct SimplelstSupplyProvider;

impl SimplelstSupplyProvider {
	pub fn set_lst_supply(supply: Balance) {
		lstSupply::set(supply);
	}
...
impl lst_voting::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type RuntimeOrigin = RuntimeOrigin;
	type RuntimeCall = RuntimeCall;
...
pub fn new_test_ext() -> sp_io::TestExternalities {
	...
	lst_voting::GenesisConfig::<Runtime> {
