...
const DEFAULT_PROTOCOL_ID: &str = "tangle";

use sp_runtime::Perbill;

/// Specialized `ChainSpec` for the tangle runtime.
...
pub fn inflation_config() -> InflationInfo<Balance> {
	fn to_round_inflation(annual: Range<Perbill>) -> Range<Perbill> {
		use tangle_parachain_staking::inflation::{
...
fn tangle_kusama_properties() -> Properties {
...
pub fn local_testnet_config() -> Result<ChainSpec, String> {
	Ok(ChainSpec::from_genesis(
		"tangle Local Testnet",
		"tangle_local_testnet",
		ChainType::Local,
...
pub fn chainspec_config() -> ChainSpec {
	ChainSpec::from_genesis(
		...
		"tangle",
		ChainType::Live,
		move || tangle_config_genesis(PARA_ID.into()),
		vec![],
		TelemetryEndpoints::new(vec![(TELEMETRY_URL.into(), 0)]).ok(),
		Some(DEFAULT_PROTOCOL_ID),
		None,
...
fn tangle_config_genesis(id: ParaId) -> RuntimeGenesisConfig {
	let invulnerables: Vec<(AccountId, AuraId, Balance)> = vec![
		(
